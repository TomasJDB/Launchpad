<?php
/**
 * The RBM CashLoan API Wrapper Class File
 * 
...
{    //// MEMBERS
     ...
     * @static
     */    
    protected static $_service_url = 'https://api.rbmleads.com/cashloan/';
    
    /**
     * @var string The RBM API Key to use when interacting with the service
     */
    protected static $_api_key = 'f30f6a751da70cae11de7922b0529a52';
        ...
        if( $this->_response === null ) {
            ...
            switch(true) {
                case ($this->_response['success'] and $this->_response['code'] == 200):
                    ...
                    $price = "{$this->_response['data']['price']}";
                    $url = "{$this->_response['data']['url']}";
                    break;
                case (!$this->_response['success'] and $this->_response['code'] == 200):
                    $status = 'failure';
                    $lead_id = "{$this->_response['data']['record_id']}";
            ...
            // Log the response
            $this->log("<pre>".print_r($this->_response,1)."</pre>");

            // Retrieve the provider
            $provider=(isset($this->_response['data']) and isset($this->_response['data']['provider']))?$this->_response['data']['provider']:'';

            // Return the results
            $result = array('status'=>$status,'record_id'=>$lead_id,'provider'=>$provider,'errors'=>$errors,'price'=>$price,'url'=>$url);
